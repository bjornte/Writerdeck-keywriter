...
Window {
    ...
    // Sliding window start so Up/Down move one row (not a whole page flip).
    property int lobbyFilesWindowStart: 0
    ...
    function lobbyFilesPageIndex() {
        var ps = Math.max(1, lobbyFilesPageSize)
        return Math.floor(Math.max(0, lobbyFilesWindowStart) / ps)
    }

    function lobbyFilesPageStart() {
        return Math.max(0, lobbyFilesWindowStart)
    }

    function lobbyFilesClampWindow() {
        var ps = Math.max(1, lobbyFilesPageSize)
        var count = lobbyNotesModel.count
        if (count <= 0) {
            lobbyFilesWindowStart = 0
            return
        }
        var maxStart = Math.max(0, count - ps)
        if (lobbyFilesIndex < lobbyFilesWindowStart)
            lobbyFilesWindowStart = lobbyFilesIndex
        else if (lobbyFilesIndex >= lobbyFilesWindowStart + ps)
            lobbyFilesWindowStart = lobbyFilesIndex - ps + 1
        if (lobbyFilesWindowStart < 0)
            lobbyFilesWindowStart = 0
        if (lobbyFilesWindowStart > maxStart)
            lobbyFilesWindowStart = maxStart
    }

    function lobbyFilesSetIndex(i) {
        var last = Math.max(0, lobbyNotesModel.count - 1)
        lobbyFilesIndex = Math.max(0, Math.min(last, i))
        lobbyFilesClampWindow()
    }
